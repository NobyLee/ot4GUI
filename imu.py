...
class IMU():
    ...
    # 当 FrameState 为 1 时，表示接收到加速度数据，将数据存储到 ACCData 数组中，并进行校验。
    # 当 FrameState 为 2 时，表示接收到陀螺仪数据，将数据存储到 GYROData 数组中，并进行校验。
    # 当 FrameState 为 4 时，表示接收到端口数据，将数据存储到 PortData 数组中，并进行校验，然后将相关数据写入文件 f。
    ...
    def start(self):
        ...
        if self.imuPort=='null': # 如果没有传入端口号
        ...
        with open(self.fileName+'.txt', 'w', encoding='utf-8') as f:
            ...
            while (self.RecordingSign != False):
                datahex = ser.read(33)  # 读取数据，一帧数据应该有33个字节
                self.DueData(datahex, f)    # 解包数据，并使用f文件句柄存储数据
...
def get_acc(datahex):
    ...
    # 除以32768.0是因为数据是有符号的16位整数，范围是-32768到32767，而除以32768.0可以将其缩放到-1到1之间
